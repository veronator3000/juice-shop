...
export function searchProducts () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    // исправлена SQL-инъекция: пользовательский ввод больше не подставляется
    // напрямую в SQL-запрос, вместо этого используется параметризованный запрос
    const searchValue = `%${criteria}%`
    models.sequelize.query(
        'SELECT * FROM Products WHERE ((name LIKE :criteria OR description LIKE :criteria) AND deletedAt IS NULL) ORDER BY name',
        {
          replacements: { criteria: searchValue }
        }
    )
      .then(([products]: any) => {
